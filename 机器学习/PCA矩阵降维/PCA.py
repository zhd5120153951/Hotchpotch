...
def PCA_2D():
    data_2d = spio.loadmat("./机器学习/PCA矩阵降维/data.mat")
    X = data_2d['X']
    m = X.shape[0]
...
def PCA_faceImage():
    print(u'加载图像数据.....')
    data_image = spio.loadmat('./机器学习/PCA矩阵降维/data_faces.mat')
    X = data_image['X']
        ...
        for j in range(cols_count):
            ...
            display_array[pad + i * (height + pad):pad + i * (height + pad) + height,
                          pad + j * (width + pad):pad + j * (width + pad) + width] = imgData[sum, :].reshape(
                              height, width, order="F") / max_val  # order=F指定以列优先，在matlab中是这样的，python中需要指定，默认以行
            sum += 1
