...
def linearRegression(alpha=0.01, num_iters=400):
    ...
    y = data[:, -1]  # y对应最后一列--存储形式[y1,y2,y3,...,y47]
    m = len(y)  # 总的数据条数
...
def featureNormaliza(X):
    ...
    #定义所需变量
    mu = np.zeros((1, X.shape[1]))  #定义一个(1,2)形状的数组--1行2列
    sigma = np.zeros((1, X.shape[1]))

    mu = np.mean(X_norm, 0)  # 求X_norm每一列的平均值（0代表列，1代表行）
    sigma = np.std(X_norm, 0)  # 求X_norm每一列的标准差
    for i in range(X.shape[1]):  # 遍历列
        X_norm[:, i] = (X_norm[:, i] - mu[i]) / sigma[i]  # 归一化所有行--即整列(样本值-均值)/标准差
