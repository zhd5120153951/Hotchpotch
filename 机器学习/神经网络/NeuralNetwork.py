...
def neuralNetwork(input_layer_size, hidden_layer_size, out_put_layer):
    data_img = loadmat_data("./机器学习/神经网络/data_digits.mat")
    X = data_img['X']
    y = data_img['y']
    '''
    ...
    start = time.time()
    result = optimize.fmin_cg(nnCostFunction,
                              initial_nn_params,
                              fprime=nnGradient,
                              args=(input_layer_size, hidden_layer_size, out_put_layer, X, y, Lambda),
                              maxiter=100)
    ...
    np.savetxt("./机器视觉/神经网络/predict.csv", res, delimiter=',')
    ...
    for i in range(rows_count):
        for j in range(cols_count):
            ...
            display_array[pad + i * (height + pad):pad + i * (height + pad) + height,
                          pad + j * (width + pad):pad + j * (width + pad) + width] = imgData[sum, :].reshape(
                              height, width, order="F")  # order=F指定以列优先，在matlab中是这样的，python中需要指定，默认以行
            sum += 1
        if sum >= m:  #超过了行数，退出当前循环
            break
...
def randInitializeWeights(L_in, L_out):
    ...
    W = np.random.rand(
        L_out, 1 + L_in) * 2 * epsilon_init - epsilon_init  # np.random.rand(L_out,1+L_in)产生L_out*(1+L_in)大小的随机矩阵
    return W
...
def checkGradient(Lambda=0):
    ...
    for i in range(nn_params.shape[0]):
        ...
        loss1 = nnCostFunction(nn_params - step.reshape(-1, 1), input_layer_size, hidden_layer_size, num_labels, X, y,
                               Lambda)
        loss2 = nnCostFunction(nn_params + step.reshape(-1, 1), input_layer_size, hidden_layer_size, num_labels, X, y,
                               Lambda)
