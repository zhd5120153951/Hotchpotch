'''
@FileName   :LogisticRegression_OneVsAll.py
@Description:手写数字识别-逻辑回归
@Date       :2022/09/07 17:10:12
@Author     :daito
...
def logisticRegression_OneVsAll():
    data = loadmat_data("./机器学习/逻辑回归/LogisticRegression/data_digits.mat")
...
def display_data(imgData):
    ...
    for i in range(10):
        for j in range(10):
            display_array[pad + i * (20 + pad):pad + i * (20 + pad) + 20, pad + j * (20 + pad):pad + j * (20 + pad) +
                          20] = (imgData[sum, :].reshape(20, 20,
...
def costFunction(initial_theta, X, y, inital_lambda):
    ...
    J = (-np.dot(np.transpose(y), np.log(h)) - np.dot(np.transpose(1 - y), np.log(1 - h)) +
         temp * inital_lambda / 2) / m  # 正则化的代价方程
    return J
