'''
@FileName   :train_mobilenet.py
@Description:mobilenet模型训练代码，训练的模型会保存在models目录下，折线图会保存在results目录下
@Date       :2023/03/04 10:51:40
@Author     :daito
...
from tensorflow.keras import Sequential, layers, optimizers, applications, models
from time import *
...
def model_create(IMG_SHAPE=(224, 224, 3), class_num=12):
    # 微调的过程中不需要进行归一化的处理
    # 加载预训练的mobilenet模型
    ...
    model = models.Sequential([
        # 进行归一化的处理
        layers.experimental.preprocessing.Rescaling(1. / 127.5, offset=-1, input_shape=IMG_SHAPE),
...
def train(epochs):
    ...
    train_data, val_data, class_names = data_load("./data/image_data", "./data/test_image_data", 224, 224, 16)
    print(class_names)
    # 加载模型
