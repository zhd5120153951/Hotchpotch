...
def start_detect():
    ...
    capture.set(cv2.CAP_PROP_POS_FRAMES, 130)  # 设置视频从第几帧开始读取

    fourcc = cv2.VideoWriter_fourcc(*"mp4v")  # 设置写入视频的编码类型
    ...
    out = cv2.VideoWriter('detect_3.mp4', fourcc, 30, (1920, 3720))  # 写入视频的格式设置
    out_mask = cv2.VideoWriter('mask_3.mp4', fourcc, 30, (1920, 3720), isColor=False)  # 写入视频的格式设置
    ...
    cv2.resizeWindow('frame', width//8, height//8)
    cv2.resizeWindow('mask', width//8, height//8)
    ...
    while True:
        ...
        if bboxs != []:
            ...
            # test
            for bbox in bboxs:
                cv2.rectangle(mask, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255, 255, 0), 2)
            trackBox, points = sort.update(bboxs)
        else:
            # test
            trackBox, points = sort.update()
        ...
        for bbox in trackBox:
            ...
            # 画出历史位置
            for box_history in points[str(bbox[4]-1)]:
                box_h = [int(box_history[i]) for i in range(4)]
                cv2.rectangle(frame, (box_h[0], box_h[1]), (box_h[2], box_h[3]), (0, 0, 255), 2)
