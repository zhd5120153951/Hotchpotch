'''
@FileName   :diff_frame_10.py
@Description:帧差法--差10帧--1th和10th差
...
# 循环处理视频帧
while True:
    video.set(cv2.CAP_PROP_POS_FRAMES, frame+10)
    # 读取下一帧
    ...
    threshold = cv2.morphologyEx(threshold, cv2.MORPH_ERODE, kernel)
    threshold = cv2.morphologyEx(threshold, cv2.MORPH_DILATE, kernel)
