'''
@FileName   :test.py
@Description:yolov5-fire模型部署推理
@Date       :2023/08/21 09:42:14
@Author     :daito
...
def draw_numpy_fire(image, boxes, masks=None, classes_ids=None, conf_scores=None):
    ...
    if classes_ids is not None and conf_scores is not None:
        classes_ids = classes_ids.astype(np.int8)
        cv2.putText(image,COCO_CLASSES[classes_ids+1]+':'+str(round(conf_scores,2)),(x1,y1-5),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),thickness=1)
        if masks is not None:
            mask = masks[:, :,:]
