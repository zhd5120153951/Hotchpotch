    ...
    def detect(self, im, age):
        ...
        for det in pred:
            if det is not None and len(det):
                ...
                for *x, conf, cls_id in det:
                    ...
                    pred_boxes.append((x1, y1, x2, y2, lbl, str(count + 1), conf))
                    count += 1
        ...
        if healthy_rate == 1:
            ...
            if virus_label > 0:
                if age == 0:
                    if virus_rate < 0.3:
                        image_info_many[str(virus_per)] = [
                            "疑似为血液性脓病",
                            "1.巡视蚕座，拈除病弱小蚕并无害化处理。\n 2.每天用防僵粉或新鲜石灰粉对蚕体蚕座消毒两次。\n 3.用消毒液对蚕室蚕具及周围环境进行消毒。\n4.稀养，加强通风换气，避免高温多湿。\n5.查明病原来源，避免再次感染。"
                        ]
                    else:
                        image_info_many[str(virus_per)] = [
                            "疑似为血液性脓病", "1.全部淘汰，进行无害化处理。\n2.对蚕室蚕具和养蚕环境进行彻底消毒。\n3.查明病原来源，避免再次感染。\n4.重新饲养下一批蚕。"
                        ]
                if age == 1:
                    if virus_rate < 0.4:
                        image_info_many[str(virus_per)] = [
            ...
            if fungus_label > 0:
                if age == 0:
                    if fungus_rate < 0.1:
                        image_info_many[str(fungus_per)] = [
                            "疑似为微粒子疾病",
                            "1.巡视蚕座，拈除病弱小蚕并无害化处理。\n 2.给桑前用新鲜石灰粉对蚕体蚕座消毒。\n3.用消毒液对蚕室蚕具及周围环境进行消毒。\n4.良桑饱食,增强蚕体质。\n5.查明病原来源，避免再次感染。"
                        ]
                    else:
                        image_info_many[str(fungus_per)] = [
                            "疑似为微粒子疾病", "1.全部淘汰，进行无害化处理。\n 2.对蚕室蚕具和养蚕环境进行彻底消毒。\n 3.查明病原来源，避免再次感染。\n4.重新饲养下一批蚕。"
                        ]
                if age == 1:
                    if fungus_rate < 0.2:
                        image_info_many[str(fungus_per)] = [
