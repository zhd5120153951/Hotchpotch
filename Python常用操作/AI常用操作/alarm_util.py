...
def send_alarm():
    ...
    for cla in det_class:
        if cla == special_name:
            # D:/yunbian_3.0_git/results/leave_sleep_detection
            alarm_data = result_path(setting.project_result_dir, cla)
            alarm_data_list = os.listdir(
                result_path(setting.project_result_dir, cla))
            for data in alarm_data_list:
                # D:/yunbian_3.0_git/results/leave_sleep_detection/2022-09-28
                alarm_data_path = result_path(alarm_data, data)
                # D:/yunbian_3.0_git/results/leave_sleep_detection/2022-09-28/txt/unsend
                alarm_data_txt_unsend = result_path(
                    alarm_data_path, 'txt/unsend')
                # D:/yunbian_3.0_git/results/leave_sleep_detection/2022-09-28/txt/sended
                alarm_data_txt_sended = result_path(
                    alarm_data_path, 'txt/sended')
                ...
                if os.listdir(alarm_data_txt_unsend) == 0:
                    ...
                    for unsend_alarm in os.listdir(alarm_data_txt_unsend):
                        # ---------------------------------组装发送告警信息--------------------------------- #
                        # 获取告警图片目录与告警原图目录
                        # ../results/Y-m-d/txt/unsend/unsend_2022-09-28_09_46_56_leave_22222222222222222.txt
                        with open(result_path(alarm_data_txt_unsend, unsend_alarm), 'r', encoding='utf-8') as F:
                            alarm_dic = F.read()
                        ...
                        if os.path.exists(alarmImage_path) and os.path.exists(alarmOriginalImage_path):
                            ...
                            base64_alarmImage = base64.b64encode(
                                byte_alarmImage).decode("ascii")
                            ...
                            base64_alarmOriginalImage = base64.b64encode(
                                byte_alarmOriginalImage).decode("ascii")
                            alarm_dic['alarmOriginalImage'] = base64_alarmOriginalImage
                        else:
                            shutil.rmtree(result_path(
                                alarm_data_txt_unsend, unsend_alarm))
                        # ------------------------------------------------------------------------------------ #
                        # 请求告警url,发送告警
                        alarm_list = []
                        alarm_list.append(alarm_dic)
                        response = requests.post(
                            setting.ALARM_PATH + '/alarm/algorithm-alarm/add', json=alarm_list)
                        if response.status_code == 200:
                            if not os.path.exists(alarm_data_txt_sended):
                                os.makedirs(alarm_data_txt_sended)
                            # 移动文件txt/unsend至txt/sended
                            shutil.move(alarm_data_txt_unsend + '/' +
                                        unsend_alarm, alarm_data_txt_sended)
                            logger.info('Sending alarm seccessfully.')
                        else:
                            logger.info(
                                f'The alarm service return exception information, the HTTP error code is {response.status_code}.')
                            pass
                        pass
                    pass
                pass
    ...
    if setting.TEST_FLAG == 0:
        try:
            for devide_code in setting.NUM:
                num_dic = {'detectionTimes': f"{setting.NUM[devide_code]}",
                           'appId': f"{setting.APP_ID}", 'deviceId': f"{devide_code}"}
                logger.debug(num_dic)
                response = requests.post(
                    setting.SYNC_PATH + '/sync/appDeviceModel/updateDetectionTimes', json=num_dic)
                logger.debug(f"post请求返回结果{response.text}")
                if response.status_code == 200:
                    # 如果发送成功，则清零，否则一直计数。
                    logger.info(
                        f'应用id: {setting.APP_ID}， 设备编码: {devide_code}， Send number of images successfully.Number of {setting.NUM[devide_code]}')
                    setting.NUM[devide_code] = 0
        except Exception as e:
            raise Exception(
                f"Failed to connect to the detect img_num service. The error message is {e}.")
    else:
        for devide_code in setting.NUM:
            logger.info(
                f'应用id: {setting.APP_ID}， 设备编码: {devide_code}， send number of images successfully.number of {setting.NUM[devide_code]}')
...
def package_business_alarm(alarm_info, alarm_type, img_alarm, img_ori, device_id, alarm_label):
    ...
    special_name = os.getcwd().replace('\\', '/').split('/')[-1]
    # D:/yunbian_3.0_git/results/leave_sleep_detection/2022-09-28
    save_img_data_dir = result_path(
        setting.project_result_dir, special_name + '/' + local_time)
    save_txt_data_dir = result_path(
        setting.project_result_dir, special_name + '/' + local_time)
    # D:/yunbian_3.0_git/results/leave_sleep_detection/2022-09-28/img
    save_img_dir = result_path(save_img_data_dir, setting.alarm_img_dir)
    os.makedirs(save_img_dir, exist_ok=True)
    save_txt_dir = result_path(save_txt_data_dir, setting.alarm_dic_dir)
    # D:/yunbian_3.0_git/results/leave_sleep_detection/2022-09-28/txt/unsend
    save_txt_dir = result_path(save_txt_dir, 'unsend')
    os.makedirs(save_txt_dir, exist_ok=True)
    # 路径不能有空格，不能有冒号，否则图片无法保存。
    alarm_time_re = str(alarm_time).replace(' ', '_')
    alarm_time_re = alarm_time_re.replace(':', '_')
    # 图片保存到容器内部
    img_alarm_name = save_img_dir + \
        f'/{alarm_time_re}_img_alarm_{alarm_label}_{device_id}.jpg'
    img_ori_name = save_img_dir + \
        f'/{alarm_time_re}_img_ori_{alarm_label}_{device_id}.jpg'
...
def cap_img_alarm():
    if setting.TEST_FLAG == 0:
        if setting.alarm_queue.qsize() > 0:
            ...
            try:
                response = requests.post(
                    setting.ALARM_PATH + '/alarm/algorithm-alarm/add', json=img_alarm_dic)
                if response.status_code == 200:
                ...
                raise Exception(
                    f"Failed to connect to the alarm service. The error message is {e}.")
