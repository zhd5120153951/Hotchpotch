...
class PriorBox(object):
    def __init__(
        ...
        self.feature_map_2th = [int(int((self.in_h + 1) / 2) / 2), int(int((self.in_w + 1) / 2) / 2)]
    ...
    def decode(self, loc: np.ndarray, conf: np.ndarray) -> np.ndarray:
        ...
        '''
        # get bboxes
        bboxes = np.hstack((self.priors[:, 0:2] + loc[:, 0:2] * self.variance[0] * self.priors[:, 2:4],
        ...
        # get landmarks
        landmarks = np.hstack((self.priors[:, 0:2] + loc[:, 4:6] * self.variance[0] * self.priors[:, 2:4],
