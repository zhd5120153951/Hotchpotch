...
class lffdface():
    def __init__(self, version=2):
        if version == 2:
            self.net = cv2.dnn.readNetFromCaffe('lffdface/symbol_10_320_20L_5scales_v2_deploy.prototxt',
                                                'lffdface/train_10_320_20L_5scales_v2_iter_1000000.caffemodel')
            ...
            self.net = cv2.dnn.readNetFromCaffe('lffdface/symbol_10_560_25L_8scales_v1_deploy.prototxt',
                                                'lffdface/train_10_560_25L_8scales_v1_iter_1400000.caffemodel')
    ...
    def detect(self,
        ...
        for i in range(self.num_output_scales):
            ...
            RF_center_Xs = np.array([
                self.receptive_field_center_start[i] + self.receptive_field_stride[i] * x
                for x in range(score_map.shape[1])
            ])
            RF_center_Xs_mat = np.tile(RF_center_Xs, [score_map.shape[0], 1])
            RF_center_Ys = np.array([
                self.receptive_field_center_start[i] + self.receptive_field_stride[i] * y
            ...
            for idx in range(select_index[0].size):
                bbox_collection.append(
                    (x_lt_mat[select_index[0][idx], select_index[1][idx]], y_lt_mat[select_index[0][idx],
                                                                                    select_index[1][idx]],
                     x_rb_mat[select_index[0][idx],
                              select_index[1][idx]], y_rb_mat[select_index[0][idx],
                                                              select_index[1][idx]], score_map[select_index[0][idx],
                                                                                               select_index[1][idx]]))
        ...
        if NMS_flag:
            ...
            for i in range(final_bboxes.shape[0]):
                xmin, ymin, xmax, ymax = final_bboxes[i, :4].astype(np.int64)
            ...
            for i in range(bbox_collection_np.shape[0]):
                xmin, ymin, xmax, ymax = bbox_collection_np[i, :4].astype(np.int32)
                cv2.rectangle(drawimg, (xmin, ymin), (xmax, ymax), (0, 0, 255), thickness=2)
                face_rois.append(image[ymin:ymax, xmin:xmax])
            return drawimg, face_rois
    ...
    def get_face(self,
        ...
        if NMS_flag:
            ...
            boxs, face_rois = [], []
            for i in range(bbox_collection_np.shape[0]):
                xmin, ymin, xmax, ymax = bbox_collection_np[i, :4].astype(np.int64)
                boxs.append((xmin, ymin, xmax, ymax))
                face_rois.append(image[ymin:ymax, xmin:xmax])
            return boxs, face_rois
